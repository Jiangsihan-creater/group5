...
from dvadmin.system.models import Users, Role, Dept, UsersRole
...
class RegisterSerializer(TokenObtainPairSerializer):
    ...
    def validate(self, attrs):
        ...
        if user is not None:
            raise CustomValidationError("用户名已经存在")
        # 将最新的token保存到用户表
        Dept.objects.filter()
        data = Users.objects.create_user(username, "", password, name=name, dept_id=dept, is_active=is_active)
        UsersRole.objects.create(user_id=data.id, role_id=role[0])
        return {"code": 2000, "msg": "注册成功", "data": {}}
