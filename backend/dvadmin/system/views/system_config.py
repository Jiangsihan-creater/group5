...
from dvadmin.utils.spam_calssification_GloVe import _train

from dvadmin.utils.predict import predict_email_category

# from dvadmin.utils.test import _train, _test
...
class SystemConfigViewSet(CustomModelViewSet):
    ...
    def get_detail_by_id(self, request):
        ...
        body = request.query_params
        id = body.get('id', 0)
        instance = SystemConfig.objects.filter(id=id).first()
        if instance is None:
            return ErrorResponse(msg="未获取到关联信息")
        result = {"id": instance.id, "value": instance.value}
        return DetailResponse(msg="查询成功", data=result)

    def train(self, request):
        body = request.query_params
        proportion = body.get('proportion', 0.5)
        ...
        if epochs is not None and epochs != "":
            epochs = int(epochs)
        # _train(proportion, "")
        dic = _train(proportion, data_path, batch_size=batch_size, epochs=epochs)
        return DetailResponse(msg="训练成功", data=dic)

    def test(self, request):
        body = request.query_params
        data_path = body.get('data_path', "")
        # _train(proportion, "")
        predict_result = predict_email_category(data_path)
        return DetailResponse(msg="识别完成", data=predict_result)

    def get_data_content(self, request):
        body = request.query_params
        data_path = body.get('data_path', "")
        # _train(proportion, "")
        df = pd.read_csv(data_path)
        email = df['Email']
        return DetailResponse(msg="请求成功", data=email)
